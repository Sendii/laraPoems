...
haloo <b>{{$user->name}}</b>
<div>{{ $user->name }}</div>
@if ($user->follows->where('user_id', Auth::user()->id)->count() == 0 && $user->id != Auth::user()->id)
<form action="{{url('profile/'. $user->id.'/follow')}}" method="POST">
	@csrf
...
@elseif(!$user->follows->where('user_id', Auth::user()->id)->count() == 0 && $user->id != Auth::user()->id)
<form action="{{url('profile/'. $user->id.'/unfollow')}}" method="POST">
	@csrf
	@method('DELETE')
...
@endif
Followers : 
<a href="#" data-toggle="modal" data-target="#m_followers">
	{{$user->totalfollowers->count()}}
</a><br>
Followings : 
<a href="#" data-toggle="modal" data-target="#m_followings">
	{{$user->totalfollowings->count()}}
</a>
